...
{
#ifdef WIN32
	return _min + static_cast <double> (std::rand()) / (static_cast <double> (RAND_MAX / (_max - _min)));
#else
...
{
	if (seed == 0) { srand(static_cast <unsigned> (time(0))); }
	else{ srand(seed); }
...
{
	size_t k, i, j, size2 = _size * _size;
	// simple smoothing
	for (k = 0; k < 2; k++) {
		for (i = 0; i < _size; i++) {
			for (j = 0; j < _size; j++) {
...
bool DiamondSquareNoise::diamond(unsigned int subdivide)
{
	...
	for (i = 0; i * squaresize < _size - 1; i++) {
		for (j = 0; j * squaresize < _size - 1; j++) {
			...
			indexBR = (j + 1) * squaresize * _size + (i + 1) * squaresize;
