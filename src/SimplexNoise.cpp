...
void SimplexNoise::init(double maxx, double maxz, double rangemin, double rangemax, int seed)
{
	_min = rangemin;
	_max = rangemax;
	if (seed == 0) { srand(static_cast <unsigned> (time(0))); _seed = rand(); }
	else{ _seed = seed; }
	_worldScale = maxx;
	if (maxz > maxx) {
		_worldScale = maxz;
...
SimplexNoise::SimplexNoise(int tilesRes, int amp, int gain, bool bamp, bool bgain) : _min(0), _max(0)
{
	tilesResolution = tilesRes;
	a_Amp = amp;
	a_Gain = gain;
	bAmp = bamp;
	bGain = bgain;
...
#define TEST
	...
	double gain;
	double amplitude;
	//const double gain = 4 * (tilesResolution * 4);//plateau unless you have low gain
	//const double gain = 1;//below 2 it's not working
	if (bGain == true)
	{
		gain = 4 * a_Gain;//more gain = ondulated plateau 
		//unless you have low amplitude which would end up being a plateau not being ondulated
	}
	else
	{
		gain = 4 / a_Gain;//more gain = ondulated plateau 
